...
export const uploadImage = async (req, res) => {
  try {
    ...
    //image params
    const params = {
      Bucket: "skilldekho.com-bucket",
      Key: `${nanoid()}.${type}`, //look something like ddxuhu.jpeg
      Body: base64Data,
      ACL: "public-read",
...
export const removeImage = async (req, res) => {
  try {
    ...
    const params = {
      Bucket: image.Bucket,
      Key: image.Key, //look something like ddxuhu.jpeg
    };

    //send remove reques to s3
...
export const create = async (req, res) => {
  ...
  try {
    ...
    if (alreadyExist) return res.status(400).send("Title taken");
...
export const uploadVideo = async (req, res) => {
  try {
    ...
    if (!video) return res.status(400).send("No video uploaded");
...
export const freeEnrollment = async (req, res) => {
  try {
    ...
    //find the user by id and add the course
...
export const paidEnrollment = async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment", // specify the mode as "payment" or "subscription" here
